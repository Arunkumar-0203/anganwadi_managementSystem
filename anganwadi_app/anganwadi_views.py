...
class pregnant_acceptedfoodbooking(TemplateView):
    ...
    def post(self,request,*arg,**kwargs):
        ...
        print(id)
        Anganwadi = anganawadi.objects.get(user_id=self.request.user.id)
        data = tbl_pfoodbooking.objects.filter(Anganwadi=Anganwadi.id,pfoodbooking_status="anganwadi request accepted",pfoodbooking_date__range=[starting, enddate])
...
class RejectedListPwomen(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        id = self.request.user.id
        Anganawadi=anganawadi.objects.get(user_id=self.request.user.id)
        pregnant= tbl_pwomens.objects.filter(user__last_name=0,location_id_id=Anganawadi.location_id,pwomen_isactive='reject')
        print(pregnant)
        context['pregnant']=pregnant
        return context
